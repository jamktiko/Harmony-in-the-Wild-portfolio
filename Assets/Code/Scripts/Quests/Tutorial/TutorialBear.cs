...
public class TutorialBear : MonoBehaviour
{
    ...
    private void Start()
    {
        CheckDialogueProgressChanges(_questInfoForPoint);
    ...
    {
        if (QuestManager.Instance.CheckQuestState(_questInfoForPoint) == QuestState.Finished)
        {
            return;
        }
    ...
    private void CheckDialogueProgressChanges(QuestScriptableObject questObject)
    {
        if (questObject == _questInfoForPoint)
        {
    ...
    {
        _currentDialogueIndex = QuestManager.Instance.GetQuest(_questInfoForPoint).GetCurrentQuestStepIndex() - 1;
    }
