...
public class TutorialBearMovement : MonoBehaviour
{
    ...
    private void CheckMovementActivation(QuestScriptableObject questObject)
    {
        if (questObject == _tutorialQuestSo)
        {
            Invoke(nameof(EnableMovement), 0.5f);
        }
    }
    ...
    {
        if (QuestManager.Instance.GetQuest(_tutorialQuestSo).GetCurrentQuestStepIndex() == 3)
        {
            _canMove = true;
