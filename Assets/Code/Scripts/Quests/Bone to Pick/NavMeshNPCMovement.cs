...
public class NavMeshNpcMovement : MonoBehaviour
{
    ...
    private void EnableMovement(DialogueQuestNpCs questCharacterToMove)
    {
        ...
        {
            if (QuestManager.Instance.GetQuest(_questSo).GetCurrentQuestStepIndex() == _questStepIndexToAllowMovement)
            {
                _canMove = true;
    ...
    {
        if (QuestManager.Instance.CheckQuestState(_questSo) == QuestState.CanFinish)
        {
