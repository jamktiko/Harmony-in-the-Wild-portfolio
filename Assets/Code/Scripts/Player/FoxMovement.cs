...
public class FoxMovement : MonoBehaviour
{
    ...
    [FormerlySerializedAs("cameraMovement")]
    [Header("Movement")]
    ...
    [SerializeField] private ParticleSystem[] _landingEffects;
    ...
    [FormerlySerializedAs("telegrabEffect")]
    [Header("VFX")]
    ...
    private bool _previouslyInWater;
    ...
    private void SetMovementSpeed(ref float speed, ref float modifier, bool isSnowDiveUnlocked)
    {
        ...
        if (IsInWater())
        {
            _previouslyInWater = true;
            speed = Swimming.Instance.SwimSpeed;
            Rb.useGravity = true;
            playerAnimator.State = FoxAnimationState.Swimming;
        }
        else if(_previouslyInWater)
        {
            // Resetting the animator state when exiting water
            _previouslyInWater = false;
            playerAnimator.State = FoxAnimationState.Default;
        }
    ...
    public bool IsOnSlope()
    {
        ...
        if (Physics.Raycast(FoxMiddle.position + Vector3.up*0.42f, Vector3.down, out SlopeHit, _playerHeight, _groundLayerMask))
        {
            float angle = Vector3.Angle(SlopeHit.normal, Vector3.up);
            Debug.DrawRay(FoxMiddle.position, Vector3.down, Color.cyan);
            return angle > 1.5f && angle < 45;
    ...
    private void SitAndLay()
    {
        ...
        if (playerAnimator.State == FoxAnimationState.Rest && !playerAnimator.Sitting && !playerAnimator.Laying)
        {
            playerAnimator.State = FoxAnimationState.Default;
        }
    }
    ...
    void OnDrawGizmos()
    {
        ...
        Gizmos.DrawWireSphere(FoxMiddle.position, _boxSize.y);
    }
    #endregion
