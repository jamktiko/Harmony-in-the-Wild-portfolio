...
public class QuestMenuManager : MonoBehaviour
{
    ...
    private void OnEnable()
    {
        ...
        _activeQuests = QuestManager.QuestMap.Where(x => x.Value.State == QuestState.CanStart || x.Value.State == QuestState.InProgress).Select(x => x.Value).OrderByDescending(x => x.State).ToList();
        foreach (Quest x in _activeQuests)
        {
            Button spawnableButton = _button;
            spawnableButton.name = x.Info.DisplayName;
