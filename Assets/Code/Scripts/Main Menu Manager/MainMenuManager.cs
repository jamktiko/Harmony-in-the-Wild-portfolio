...
public class MainMenuManager : MonoBehaviour
{
    ...
    private void LoadSavedGame()
    {
        ...
        var questObject = QuestManager.GetQuestObject("Tutorial");
        if (QuestManager.Instance.CheckQuestState(questObject) != QuestState.Finished)
        {
            // check the current quest step state to see if there's still something to be done in Bear Cave
            if (QuestManager.Instance.GetQuest(questObject).GetCurrentQuestStepIndex() < 4)
            {
    ...
    public void StartNewGame()
    {
        ...
        QuestManager.Instance.CreateQuestMap();
